      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl bg-white/5 p-8 border border-white/10 backdrop-blur-md shadow-2xl">
        ...
        <div className="flex justify-center space-x-4 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${!isAdminRegister ? 'bg-primary-600 text-white' : 'bg-white/10 text-text-muted-dark hover:bg-white/20'}`}
          >
            User
          </button>
          <button
        ...
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            ...
            >
              {currentIsRegistering ? 'Creating account...' : 'Create account'}
            </button>
          </div>
        </form>
      </div>
